...
const NAV_ITEMS = [
  { to: '/agent/active', icon: 'local_shipping', label: 'Active Deliveries' },
    ...
    <div className="bg-background text-on-background h-screen w-full overflow-hidden flex font-sans">
      ...
      <aside className="hidden md:flex bg-white h-screen w-64 border-r border-slate-200 shadow-sm flex-col py-6 z-20 shrink-0">
        <div className="px-6 mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold text-primary">Horizon Agent</h1>
          ...
          {NAV_ITEMS.map(({ to, icon, label }) => {
            ...
            return (
              <Link
                key={to}
                to={to}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive ? 'bg-blue-50 text-primary' : 'text-slate-500 hover:text-primary hover:bg-slate-50'}`}
              >
        ...
        <div className="px-3 mt-6 border-t border-slate-200 pt-4">
          <div className="flex items-center gap-3 px-3 py-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-on-surface truncate">{user?.email || 'Agent'}</p>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="bg-white/80 backdrop-blur-md w-full border-b border-slate-200 flex justify-between items-center px-6 h-16 sticky top-0 z-40">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-primary tracking-tight">QuickBite</h2>
          </div>
          <div className="flex items-center gap-3">
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-xs font-medium text-emerald-700">Online</span>
            <div className="w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center text-sm font-semibold ml-2">
              {(user?.email?.[0] || 'A').toUpperCase()}
            </div>
          </div>
        </header>
        ...
        {NAV_ITEMS.map(({ to, icon, label }) => {
          ...
          return (
            <Link
              key={to}
              to={to}
              className={`flex flex-col items-center justify-center rounded-xl px-3 py-1.5 transition-transform active:scale-90 ${isActive ? 'text-primary' : 'text-slate-400'}`}
            >
