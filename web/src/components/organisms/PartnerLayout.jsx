          ...
          {NAV_ITEMS.map(({ to, icon, label }) => {
            ...
            return (
              <Link
                key={to}
                to={to}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 ${isActive ? 'bg-blue-50 text-primary border-r-2 border-primary' : 'text-slate-500 hover:text-primary hover:bg-slate-50'}`}
              >
        ...
        <div className="px-3 mt-6 border-t border-slate-200 pt-4">
          <div className="flex items-center gap-3 px-3 py-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-on-surface truncate">{user?.email || 'Partner'}</p>
