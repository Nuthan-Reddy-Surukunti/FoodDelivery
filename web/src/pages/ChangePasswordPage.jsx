...
export const ChangePasswordPage = () => {
  ...
  const formContent = (
    <div className="w-full max-w-md mx-auto space-y-stack-lg">
      {/* Mobile Brand Header */}
  ...
  if (user?.role === 'RestaurantPartner') {
    return <PartnerLayout title="Security">{formContent}</PartnerLayout>
  }

  if (user?.role === 'Admin') {
    return <AdminLayout title="Security">{formContent}</AdminLayout>
  }

  if (user?.role === 'DeliveryAgent') {
    return <AgentLayout title="Security">{formContent}</AgentLayout>
  }
